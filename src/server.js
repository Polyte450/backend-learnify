...
app.get("/api/applications", requireAuth, async (req, res, next) => { try { const where = req.auth.role === "ADMIN" ? {} : req.auth.role === "OPPORTUNITY_GIVER" ? { opportunity: { createdById: req.auth.sub } } : { userId: req.auth.sub }; const applications = await prisma.application.findMany({ where, include: { opportunity: true, user: true }, orderBy: { createdAt: "desc" } }); res.json({ applications: applications.map(publicApplication) }); } catch (error) { next(error); } });
app.patch("/api/applications/:id", requireAuth, async (req, res, next) => { try { const data = z.object({ status: z.string().transform((value) => value.toUpperCase()).pipe(z.enum(["PENDING", "REVIEW", "APPROVED", "REJECTED"])) }).parse(req.body); const existing = await prisma.application.findUnique({ where: { id: req.params.id }, include: { opportunity: true } }); if (!existing) return res.status(404).json({ message: "Application not found" }); if (req.auth.role !== "ADMIN" && !(req.auth.role === "OPPORTUNITY_GIVER" && existing.opportunity.createdById === req.auth.sub)) return res.status(403).json({ message: "Insufficient permissions" }); const application = await prisma.application.update({ where: { id: req.params.id }, data, include: { opportunity: true, user: true } }); res.json({ application: publicApplication(application) }); } catch (error) { next(error); } });
...
app.get("/api/admin/overview", requireAuth, requireRole("ADMIN"), async (_req, res, next) => { try { const [youth, opportunityGivers, opportunities, providers] = await Promise.all([prisma.user.count({ where: { role: "STUDENT" } }), prisma.user.count({ where: { role: "OPPORTUNITY_GIVER" } }), prisma.opportunity.count(), prisma.user.findMany({ where: { role: "OPPORTUNITY_GIVER" }, select: { id: true, name: true, email: true } })]); res.json({ youth, opportunityGivers, opportunities, providers: providers.map((provider) => ({ ...provider, disabled: false, subscription: { plan: "trial", reminderNumber: 1 } })) }); } catch (error) { next(error); } });
app.get("/api/admin/analytics", requireAuth, requireRole("ADMIN"), async (_req, res, next) => { try { const [users, providers, opportunities, applications, pendingApplications, approvedApplications] = await Promise.all([prisma.user.count(), prisma.user.count({ where: { role: "OPPORTUNITY_GIVER" } }), prisma.opportunity.count(), prisma.application.count(), prisma.application.count({ where: { status: "PENDING" } }), prisma.application.count({ where: { status: "APPROVED" } })]); res.json({ totals: { users, youth: users - providers - 1, providers, opportunities, applications }, statuses: { activeUsers: users, disabledUsers: 0, pendingApplications, approvedApplications }, plans: { trial: providers, paid: 0, expired: 0 }, activity: [] }); } catch (error) { next(error); } });
